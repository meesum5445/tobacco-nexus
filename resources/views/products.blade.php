<style>
    ...
    .filter-form {
        ...
        padding: 20px;
        margin-top:50px;
        background-color: #6f4e37; /* Medium brown background */
        border-radius: 10px;
        box-shadow: 0 0 10px rgba(0, 0, 0, 0.5);
        color: #ffffff;
    }
    ...
    .filter-form button {
        ...
        font-size: 1em;
        color: #ffffff;
        background-color: #8a5a44; /* Darker brown button */
        border: none;
        border-radius: 5px;
        cursor: pointer;
        transition: background-color 0.3s ease;
    ...
    .filter-form button:hover {
        background-color: #6f4e37; /* Medium brown on hover */
    }
</style>
@extends('layout')
...
@section('content')
    <form action="" class="filter-form" method="GET">
        ...
        @foreach ($listofallcategoriespassedtoview as $category)
            <label>
    ...
    <div class="container">
        @foreach ($listofproductspassedtoview as $product)
            <div class="product-item">
                <div class="product-name">{{$product->name}}</div>
                <img src="data:image/png;base64,{{ base64_encode($product->image) }}" alt="Product Image" class="product-image">
                
                <div class="product-detail"><strong>Category:</strong> {{$product->category_name}}</div>
